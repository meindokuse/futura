...
async def get_workdays(
    ...
    return await workday_service.get_schedule_filter(redis, uow, filters, date_now)
...
async def add_workday(
    ...
    workday_service = WorkService()
    await workday_service.add_employers_to_work(uow, workday,int(user.id))
...
async def delete_workday(uow: UOWDep, id: int,user: user_dep):
    workday_service = WorkService()
    await workday_service.delete_work_day(uow, id,int(user.id))
...
async def update_workday(
    ...
    workday_service = WorkService()
    await workday_service.update_work_day(uow, workday_update.id, workday_update,int(user.id))
